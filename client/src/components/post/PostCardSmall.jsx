      ...
      <Grid container>
        ...
        <Grid item xs={12}>
          <Box className={classes.smallCardLeftSide}>
            <Link to={`/post/${post._id}`} >
              {post.images.length !== 0 ? <ImageCarousel post={post} /> : ""}
            </Link>
          </Box>
        </Grid>
        ...
        <Grid item xs={12}>
          <Grid container spacing={1}>
            ...
            <Grid item xs={12}>
              <Box pt={1} flexGrow={1}>
                {post.content && <Typography sx={{ color: (theme) => { return theme.palette.text.primary } }} variant='subtitle2' className={classes.smallCardBodyContent}>{post.content}</Typography>}
              </Box>
            </Grid>
            ...
            <Grid item xs={12}>
              <Box display="flex" alignItems="center">
                ...
                <Box display="inline-flex" alignItems="center" flexDirection="column">
                  <Typography sx={{ color: (theme) => { return theme.palette.text.primary } }} variant='subtitle2' pl={0.5}>{post.user.fullname}</Typography>
                  <Typography sx={{ color: (theme) => { return theme.palette.text.primary } }} variant='caption' pl={0.5}>{" @" + post.user.username}</Typography>
                </Box>
              </Box>
            </Grid>
