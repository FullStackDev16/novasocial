...
import notification from './media/audio/notification.wav'
...
const SocketClient = () => {
    const { auth, socket, notify, settings, online, call } = useSelector(state => state)
    ...
    useEffect(() => {
        socket.on('removeLikeToClient', newPost => { dispatch({ type: POST_TYPES.UPDATE_POST, payload: newPost }) })
        return () => socket.off('removeLikeToClient')
    }, [socket, dispatch])

    // Dislikes
    useEffect(() => {
        socket.on('dislikeToClient', newPost => { dispatch({ type: POST_TYPES.UPDATE_POST, payload: newPost }) })
        return () => socket.off('dislikeToClient')
    }, [socket, dispatch])

    useEffect(() => {
        socket.on('removeDislikeToClient', newPost => { dispatch({ type: POST_TYPES.UPDATE_POST, payload: newPost }) })
        return () => socket.off('removeDislikeToClient')
    }, [socket, dispatch])
    ...
    useEffect(() => {
        socket.on('createNotifyToClient', msg => {
            ...
            if (settings.notifySound) {
                ...
                spawnNotification("@" + msg.user.username + ' ' + msg.text, msg.user.profileImage, msg.url, 'Dook')
            }
        })
        return () => socket.off('createNotifyToClient')
    }, [socket, dispatch, settings, notify.sound])
