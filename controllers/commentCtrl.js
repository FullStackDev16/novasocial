...
const commentCtrl = {
    ...
    dislikeComment: async (req, res) => {
        try {
            const comment = await Comments.find({_id: req.params.id, dislikes: req.user._id})
            if(comment.length > 0) return res.status(400).json({msg: "You already liked this comment"})

             const dislike = await Comments.findOneAndUpdate({_id: req.params.id}, {$push: {dislikes: req.user._id}}, {new: true})
    ...
    removeDislikeComment: async (req, res) => {
        try {
            const dislike = await Comments.findOneAndUpdate({_id: req.params.id}, {$pull: {dislikes: req.user._id}}, {new: true})
